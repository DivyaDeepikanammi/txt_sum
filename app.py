...
from C4 import extractive_summarization
...
options = ["lexRank", "Nltk", "TextRank", "Fuzzy-Wuzzy"]
...
elif selected_option == "Fuzzy-Wuzzy":
    st.write("You selected Fuzzy-Wuzzy!")
    text5 = st.text_area('Enter Text Below :', height=300)
    submit = st.button('Generate') 
    if submit: 
        st.subheader("Summary:")
        summary = extractive_summarization(text5)
        st.success(summary)
